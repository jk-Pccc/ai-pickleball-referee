...
class PickleballScorer:
    ...
    def process_ball_detection(self, results, frame):
        ...
        if results[0].boxes is not None and len(results[0].boxes) > 0:
            ...
            if self.last_ball_position is not None:
                ...
                # 检测球是否从一方飞到另一方
                if (last_x < self.midline_x and x_center >= self.midline_x and
                        self.is_point_in_polygon((last_x, last_y), self.court_coords) and
                        self.is_point_in_polygon((x_center, y_center), self.court_coords)):
                    self.last_hit_side = 'A'
                    print("A方击球到B方场地")

                elif (last_x >= self.midline_x and x_center < self.midline_x and
                      self.is_point_in_polygon((last_x, last_y), self.court_coords) and
                      self.is_point_in_polygon((x_center, y_center), self.court_coords)):
            ...
            if (self.last_hit_side == 'A' and self.ball_in_right and
                    y_center > self.display_height * 0.6):
                self.score_A += 1
    ...
    def update_trajectory(self, current_pos):
        ...
        if self.ball_trajectory and abs(current_pos[0] - self.ball_trajectory[-1][0]) < 3 and abs(current_pos[1] - self.ball_trajectory[-1][1]) < 3:
            return
        # 添加当前位置到轨迹
    ...
    def draw_court_and_info(self, frame):
        ...
        if len(self.ball_trajectory) >= 2:  # 至少2个点才绘制线段
            ...
            cv2.putText(frame, "Start", (self.ball_trajectory[0][0]+5, self.ball_trajectory[0][1]-5),
