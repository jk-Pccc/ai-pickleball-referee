...
class PickleBallScorer:
    ...
    def start_scoring(self):
        ...
        if self.draw_complete:
            ...
            predict_params = {"conf": 0.4, "iou": 0.4, "max_det": 3, "augment": True, "agnostic_nms": True}
        ...
        while True:
            ...
            if len(self.court_corners) > 0:
                ...
                if len(self.court_corners) > 1:
                    disp_pts = np.array(
                        [[int(x * self.scale_ratio), int(y * self.scale_ratio)] for x, y in self.court_corners],
                        np.int32,
                    ).reshape((-1, 1, 2))
            ...
            cv2.putText(
                display_frame, help_text, (20, self.frame_display_height - 20), self.font, 0.7, (255, 255, 0), 2
            )
        ...
        for result in results:
            for box in result.boxes:
                if box.cls == 0:
                    ...
                    if (
                        self.min_ball_size < ball_width < self.max_ball_size
                        and self.min_ball_size < ball_height < self.max_ball_size
                        and self.min_aspect_ratio < aspect_ratio < self.max_aspect_ratio
                    ):
                        center_x = (x1 + x2) // 2
                        center_y = (y1 + y2) // 2
                        detected_balls.append({"pos": (center_x, center_y), "conf": conf, "bbox": (x1, y1, x2, y2)})
    ...
    def draw_info(self, frame):
        ...
        cv2.putText(
            frame, f"Player A: {self.score_a}", (50, 50), self.font, self.font_scale, self.font_color, self.thickness
        )
        cv2.putText(
            frame,
            f"Player B: {self.score_b}",
            (width - 200, 50),
            self.font,
            self.font_scale,
            self.font_color,
...
if __name__ == "__main__":
    ...
    scorer = PickleBallScorer(model_path=MODEL_PATH, source=VIDEO_PATH, save_video=SAVE_VIDEO)
