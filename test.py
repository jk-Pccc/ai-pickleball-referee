...
def cv2_add_chinese_text(img, text, position, text_color=(255, 255, 255), text_size=20):
    """在OpenCV图像上绘制中文字符 使用PIL库来确保中文字体正确显示.
    """
    try:
        # 将OpenCV图像转换为PIL图像
...
class PickleBallScorer:
    ...
    def draw_court_setup_guide(self, frame):
        ...
        for i, point in enumerate(self.court_points):
            ...
            # 使用支持中文的函数绘制文本
            frame = cv2_add_chinese_text(frame, f"点{i + 1}", (point[0] + 10, point[1] - 10), (0, 255, 0), 15)
        ...
        for msg in messages:
            # 绘制带背景的文本
            # 估算文本大小
            (text_width, _text_height), _ = cv2.getTextSize(msg, self.font, 0.7, 2)
    ...
    def process_detections(self, results, frame, overlay):
        ...
        if best_box is not None:
            ...
            if ball_detected_this_frame:
                ...
                # 在调试模式下显示置信度
                if self.debug_mode:
                    overlay = cv2_add_chinese_text(
                        overlay, f"置信度: {best_conf:.2f}", (x1, y1 - 10), (0, 255, 255), 15
                    )
        ...
        # 更新球的检测状态
        self.ball_detected = ball_detected_this_frame or (
            len(self.ball_positions) > 0 and current_time - self.last_detection_time < self.detection_timeout
        )
    ...
    def update_velocity_and_acceleration(self, current_pos):
        ...
        if len(self.ball_positions) > 0:
            ...
            # 计算新速度
            new_velocity = [current_pos[0] - prev_pos[0], current_pos[1] - prev_pos[1]]
            # 应用平滑滤波
            alpha = 0.7  # 平滑系数
    ...
    def draw_info(self, overlay, info_overlay):
        ...
        # 绘制文本 - 使用支持中文的函数
        info_overlay = cv2_add_chinese_text_to_info_overlay(info_overlay, score_text, (20, 40), self.font_color, 20)
        info_overlay = cv2_add_chinese_text_to_info_overlay(info_overlay, server_text, (20, 70), self.font_color, 20)
        info_overlay = cv2_add_chinese_text_to_info_overlay(info_overlay, rally_text, (20, 100), self.font_color, 20)

        # 绘制置信度阈值信息
        info_overlay = cv2_add_chinese_text_to_info_overlay(
            info_overlay, conf_text, (20, height - 30), (0, 255, 255), 15
        )
        ...
        if self.debug_mode:
            ...
            for text in debug_texts:
                info_overlay = cv2_add_chinese_text_to_info_overlay(info_overlay, text, (20, y_pos), (255, 255, 0), 15)
                y_pos -= 20
        ...
        info_overlay = cv2_add_chinese_text_to_info_overlay(
            info_overlay, help_text, (width - 590, height - 15), (255, 255, 255), 15
        )
