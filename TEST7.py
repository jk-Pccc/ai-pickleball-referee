...
class PickleballScorer:
    def __init__(self):
        ...
        # 新增：小地图设置
        self.minimap_scale = 0.25  # 小地图缩放比例
        self.minimap_width = int(self.display_width * self.minimap_scale)
        self.minimap_height = int(self.display_height * self.minimap_scale)
        self.minimap_offset_x = 20  # 小地图距离右边界的偏移
        self.minimap_offset_y = 20  # 小地图距离上边界的偏移
        self.minimap_border_color = (255, 255, 255)  # 小地图边框颜色
        self.minimap_border_thickness = 2  # 小地图边框粗细
    ...
    def scale_coordinates_to_minimap(self, points):
        ...
        for point in points:
            ...
            x, y = point
            # 等比缩放坐标
            scaled_x = int(x * self.minimap_scale)
            scaled_y = int(y * self.minimap_scale)
    ...
    def draw_minimap(self, frame):
        ...
        # 绘制小地图中的球场边界
        if len(scaled_court_coords) == 4:
            court_points = np.array(scaled_court_coords, np.int32)
            cv2.polylines(minimap_bg, [court_points], True, (255, 0, 0), 1)
        ...
        if len(self.ball_trajectory) >= 2:
            scaled_trajectory = self.scale_coordinates_to_minimap(self.ball_trajectory)

            # 使用渐变色表示轨迹时间顺序
            for i in range(1, len(scaled_trajectory)):
                prev_pos = scaled_trajectory[i - 1]
                curr_pos = scaled_trajectory[i]

                # 根据轨迹点的新旧程度调整颜色（越新越亮）
                color_intensity = int(255 * i / len(scaled_trajectory))
                color = (color_intensity, 215, 255 - color_intensity)

                cv2.line(minimap_bg, prev_pos, curr_pos, color, 1)

            # 绘制轨迹起点和终点标记
            if len(scaled_trajectory) > 0:
                start_pos = scaled_trajectory[0]
                end_pos = scaled_trajectory[-1]

                cv2.circle(minimap_bg, start_pos, 2, (0, 255, 0), -1)  # 绿色起点
                cv2.circle(minimap_bg, end_pos, 2, (0, 0, 255), -1)  # 红色终点

        # 绘制小地图中的当前球位置
        if self.current_ball_position is not None:
            scaled_ball_pos = self.scale_coordinates_to_minimap([self.current_ball_position])[0]
    ...
    def run(self):
        ...
        print("按 'm' 切换小地图显示")
        print("按 'E' 停止/恢复计分，按 'q' A队加1分，按 'a' A队减1分")
        print("按 'w' B队加1分，按 's' B队减1分")

        frame_count = 0
        show_minimap = True  # 控制小地图显示
